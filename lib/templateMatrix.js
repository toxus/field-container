...
class templateMatrix {// extends recordTemplateClass {
  ...
  export(record, options = {}) {
    function findField(name) {
      for (let l = 0; l < record.fields.length; l++) {
        if (record.fields[l].refId == name) {
          return record.fields[l];
        }
      }
    ...
    for (let l = 0; l < this.columnCount(); l++) {
      ...
      if (typeof fieldDef.select !== 'undefined') { // we have to select a field
        throw new Error('field.select not yet implemented'); // TODO create select of fields in template
      } else { // it's bound by field name
        const dataField = findField(fieldDef.fieldName);

        if (dataField) { // we have got it, so
          for (let key of Object.keys(fieldDef.data)) {
            if (typeof dataField.key !== 'undefined') {
              result[fieldDef.data[key]] = dataField[key];
            } else {
