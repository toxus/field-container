    ...
    for (let l = 0; l < this.columnCount(); l++) {
      ...
      if (dataField) { // we have got it, so
        for (let key of Object.keys(fieldDef.data)) {
          let recKey = fieldDef.data[key];
          const isOptional = recKey.charAt(recKey.length - 1) === '?';
          if (isOptional) {
            recKey = recKey.substr(0, recKey.length - 1 );
          }
          if (typeof dataField.data[key] !== 'undefined') {
            result[recKey] = dataField.data[key];
          } else if (! isOptional) { // optional fields are not included
            result[recKey] = '';
          }
        }
