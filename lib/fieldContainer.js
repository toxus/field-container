...
class fieldContainer {
  ...
  fields(rawFields) {
    if (typeof rawFields !== 'undefined') {
      ...
      for (let l = 0; l < this._fields.length; l++) {  // rebuild fast search
        this.keys[this._fields[l].name] = l;
  ...
  add(name, type, data, usage) {
    ...
    try {
      ...
      this.addError('error', result.name, err);
      return false;
    }
  ...
  */
  ...
  diff(that) {
    ...
    for (let l = 0; l < that.count(); l++) {
      ...
      let thisField = this.get(thatField.name);
      if (thisField === false) {
        ...
        } else if (!this._compareArrays(thisField.usage, thatField.usage)) {  //
    ...
    for (let l = 0; l < this.count(); l++) {
      ...
      if (that.get(thisField.name) === false) {
  ...
  patch(diff) {
    ...
    for (let l = 0; l < diff.add().length; l++) {
      let add = diff.add()[l];
      if (typeof this._keys[add.field.name] !== 'undefined') {
        this.addError('error', add.field.name, 'duplicate name')
      } else {
        this._fields.push(add.field);
        this._keys[add.field.name] = this.count() - 1;
    ...
    for (let l = 0; l < diff.update().length; l++) {
      ...
      if (typeof this._keys[upd.field.name] === 'undefined') {
        this.add('error', upd.field.name, 'missing field to update');
      } else {
        let index = this._keys[upd.field.name];
    ...
    for (let l = 0; l < diff.delete().length; l++) {
      ...
      patch.add(del);
      if (typeof this._keys[del.field.name] === 'undefined') {
        this.add('error', del.field.name, 'missing field to delete');
      } else {
        let index = this._keys[del.field.name];
        this.delete(index);
      }
    }
    return patch;
