...
class fieldText extends fieldBase {
  ...
  parse(field) {
    if (typeof field.data === 'undefined' ) {              // need data
      ...
      field.data.value = String(field.data.value).trim(); // remove space, etc
  ...
  hasData(field) {
    ...
    return field.data.value ? (String(field.data.value).trim().length > 0) : false;
  }
   ...
   * @param field
   */
  isEqual(thisField, thatField, options) {
    return (thisField.data.value == thatField.data.value) && super.isEqual(thisField, thatField, options);
  }
